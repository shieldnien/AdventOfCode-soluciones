...
// Filtra el contenido del string para que contenga los números sólo
const regex = /[1-9]+/g


// Obtiene las coincidencias a partir del regex
const arrCoincidencias = getCoincidences(arrayStr, regex)

// Obtiene cada linea y la convierte a string
const arrParsedCoincidencias = getEveryLine(arrCoincidencias)

// Obtiene el primer y el último valor del string
const arrValues = getValue1stnLast(arrParsedCoincidencias)
...
function getCoincidences(arr, reg) {
    return arr.map((element) => {
        let coincidencias = element.match(reg);
        return coincidencias;
    })
}

function getEveryLine(arr) {
    return arr.map((element) => {
        return element.toString()
    })
}

function getValue1stnLast(arr) {
    return arr.map((element) => {
        if(element.length > 1) {
            return parseInt(element.charAt(0)+element.charAt(element.length-1))
        } else {
